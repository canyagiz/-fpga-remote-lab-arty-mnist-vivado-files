...
// =============================================================================
//
// Replaces the Zynq PS + AXI BRAM + UART0 bridge: the Raspberry Pi's UART TX
// line connects directly to a PL pin (real Pmod JA header), and this module
// receives bytes serially, writing each one straight into fmap0. No ARM core,
// no AXI, no shared BRAM, no BOOT.BIN/FSBL — the image path is 100% PL.
//
...
// 40_000_000/(115_200*16) = 21.7 ticks/sample, which truncates to 21 —
// an ~3.2% baud-rate quantization error that compounds bit-by-bit across a
// byte and was measured (via tb_uart_rx.v) to corrupt the high-order data
// bits of nearly every byte. 40_000_000/(125_000*16) = 20 ticks/sample
// EXACTLY, eliminating the error entirely. Both endpoints of this link are
// custom firmware (this module and the Raspberry Pi sender), so there's no
// external fixed-115200 peripheral constraining the choice — matching the
// baud rate to the clock is the simpler fix vs. a fractional/DDA tick
// generator. The RPi side must be configured to the same 125_000 baud
// (pyserial supports arbitrary baud rates on Linux via termios2/BOTHER).
//
